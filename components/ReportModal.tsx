...
export function ReportModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/60 px-5 backdrop-blur-sm">
      <form
        onSubmit={submit}
        className="animate-pop w-full max-w-sm space-y-4 rounded-2xl bg-cream p-7 shadow-soft"
      >
        <div>
        ...
        <div className="flex gap-2 pt-1">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 rounded-lg border border-ink/15 py-3 text-sm font-semibold text-ink-soft hover:bg-sand"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={submitting}
            className="flex-1 rounded-lg bg-terracotta py-3 text-sm font-semibold text-white hover:bg-terracotta-deep disabled:opacity-60"
          >
