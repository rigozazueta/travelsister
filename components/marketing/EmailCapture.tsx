  ...
  if (state === "done" || state === "already") {
    return (
      <div className="animate-pop rounded-2xl bg-sage/15 px-6 py-5 text-center">
        <p className="font-display text-xl font-medium text-sage-deep">
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-3 sm:flex-row">
      <input
        ...
        className="rounded-lg border border-ink/15 bg-white px-5 py-3 text-sm outline-none placeholder:text-ink-soft/60 focus:border-terracotta sm:w-40"
      />
      <input
        type="email"
        required
        ...
        className="flex-1 rounded-lg border border-ink/15 bg-white px-5 py-3 text-sm outline-none placeholder:text-ink-soft/60 focus:border-terracotta"
      />
      <button
        type="submit"
        disabled={state === "saving"}
        className="rounded-lg bg-terracotta px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-terracotta-deep disabled:opacity-60"
      >
